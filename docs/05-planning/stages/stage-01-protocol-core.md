...
**GAP NOTE (Partially resolved 2026-05-23 — host commit persistence done; effect handler + network bridge outstanding. Foundation layer resolved 2026-05-17b):** Malachite BFT was never integrated. C1 has committee sampling math but no propose/vote/commit protocol. The node binary does not produce blocks — it runs a `sleep(100ms)` counter. The state machine (C2) is genuinely implemented and works, but it is not wired into a running consensus loop. **Resolution so far:** `ConsensusDriver` implemented in `hyperfluid-consensus/src/driver.rs` — produces real blocks with state machine execution, SMT root computation, parent hash chaining. Node binary `main.rs` replaced sleep loop with async block production. Malachite `arc-malachitebft-core-*` crates at version 0.7.0-pre are now workspace dependencies (verified compiling on MSRV 1.88). **Resolution 2026-05-17b:** Malachite BFT type-level integration implemented in `hyperfluid-consensus/src/malachite.rs` — `SigningScheme` for ML-DSA-65 (~60 lines), `Context` with full Hyperfluid type wiring (Address32, BlockHeight, BlockValue, HyperfluidValidator, HyperfluidValidatorSet, HyperfluidVote, HyperfluidProposal, HyperfluidExtension) (~350 lines), 13 new tests. **Outstanding:** Effect handler trait (~300 lines), clatter network bridge (~500 lines). **Resolved 2026-05-23:** Host actor commit persistence — `BlockCommitted` handler now pushes blocks to `block_store` and updates `driver.height` (GAP-01a, 2 new tests). **Remaining:** Effect handler trait + clatter network bridge deferred to Week 9-10.
...
**GAP NOTE (CLOSED 2026-05-23 — overengineered; removed in cleanup):** FR-0183 (Stake Concentration Monitoring) was a must-have requirement. Anti-split detection code (`staking/src/graph.rs`, 455 lines) and decentralization metrics (`compute_decentralization_metrics`, 228 lines) were deleted in `checkpoint-2026-05-19-cleanup.md`. Stake concentration monitoring was judged overengineered: committee influence is stake-proportional with anti-split clustering at committee level; a separate scoring/monitoring layer is unnecessary. EIP-1559 base fee is the sole congestion mechanism. FR-0183 remains in requirements file for historical traceability but has no build task and no code — CLOSED.
