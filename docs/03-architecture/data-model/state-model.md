...
erDiagram
    ...
    TASK {
        ...
        bytes32 parent_task_id FK "references TASK (optional, zero if top-level)"
        bytes32[] depends_on "task_ids that must be Done before claim; empty for none"
        bytes32 primary_owner "agent_id with active lease (optional, zero if none)"
        bytes32 funder "agent_id that created and funded the bounty"
        string status "open | claimed | in_progress | done | decomposed"
